...
2. That is all — the feature is active automatically for any actor whose reserved tiles are set when a "change spritesheet" event is on an actor. The plugin automaticaly doubles that amount to be used as a buffer when the spritesheet changes.
